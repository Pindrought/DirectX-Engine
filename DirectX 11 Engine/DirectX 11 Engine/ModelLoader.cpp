...
HRESULT ModelLoader::LoadModel(Microsoft::WRL::ComPtr<ID3D11Device>& device, std::string & filename, std::shared_ptr<ModelData>& modeldata)
{
	modeldata = ModelManager::GetModelData(filename);
	...
	{
		return Loadp3d(device, filename, modeldata);
	}
	if (filename.find(objextension) == (filename.size() - 4))
	{
		return Loadobj(device, filename, modeldata);
	}
	LogError("3d Object File Extension not supported in file: " + filename);
	return E_NOTIMPL;
}

HRESULT ModelLoader::Loadp3d(Microsoft::WRL::ComPtr<ID3D11Device>& device, std::string & fileName, std::shared_ptr<ModelData> &modeldata)
{
	...
	infile.read((char*)modeldata->vertices.data(), sizeof(Vertex) * vertexCount);
	modeldata->indices.resize(indexCount);
	infile.read((char*)modeldata->indices.data(), sizeof(DWORD) * indexCount);
	...
	vertexBufferData.pSysMem = modeldata->vertices.data();
	HRESULT hr;
	hr = device->CreateBuffer(&vertexBufferDesc, &vertexBufferData, modeldata->vertex_buffer.GetAddressOf());
	if (hr != S_OK)
	{
	...
	indexBufferData.pSysMem = modeldata->indices.data();
	hr = device->CreateBuffer(&indexBufferDesc, &indexBufferData, modeldata->index_buffer.GetAddressOf());
	if (hr != S_OK)
	{
		LogError("Failed to call ID3D11Device::CreateBuffer for the index buffer. Error Code: " + std::to_string(hr));
		return hr;
	}
	ModelManager::StoreModelData(modeldata);
	return S_OK;
}
...
HRESULT ModelLoader::Loadobj(Microsoft::WRL::ComPtr<ID3D11Device>& device, std::string & fileName, std::shared_ptr<ModelData> &modeldata)
{	
	std::ifstream infile(fileName, std::ios::binary);
	...
	while (!infile.eof()) //while not at end of file
	{
		...
		if (strcmp(word.c_str(), "f") == 0) //if use material lib
		{
			...
			for (int i = 0; i < 3; i++)
			{
				...
				modeldata->vertices.push_back(newvert);
			}
			continue;
		}
	}
	...
	{
		modeldata->indices.push_back(i); //we'll fix this later but for now reusing vertices and just assigning a seperate index for each vert
	}
	...
	vertexBufferDesc.Usage = D3D11_USAGE_DEFAULT;
	vertexBufferDesc.ByteWidth = sizeof(Vertex)*modeldata->vertices.size();
	vertexBufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	vertexBufferDesc.CPUAccessFlags = 0;
	vertexBufferDesc.MiscFlags = 0;
	...
	ZeroMemory(&indexBufferDesc, sizeof(indexBufferDesc));
	indexBufferDesc.Usage = D3D11_USAGE_DEFAULT;
	indexBufferDesc.ByteWidth = sizeof(DWORD)*modeldata->indices.size();
	indexBufferDesc.BindFlags = D3D11_BIND_INDEX_BUFFER;
	indexBufferDesc.CPUAccessFlags = 0;
	indexBufferDesc.MiscFlags = 0;
